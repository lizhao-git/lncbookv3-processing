...
- Generic BED, GTF and annotation validators: the local `validate_bed`, `validate_gtf` and `validate_annotation` modules keep that role; bedGraph, WIG, genePred/refFlat, SAM, chain, FASTA, bigWig/bigBed, PSL, RepeatMasker `.out` and `chrom.sizes` have their own local validators (`validate_bedgraph`, `validate_wig`, `validate_genepred`, `validate_sam`, `validate_chain`, `validate_fasta`, `validate_bigtrack`, `validate_psl`, `validate_rmsk`, `validate_chrom_sizes`).
